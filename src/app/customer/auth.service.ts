...
import { Leads } from './cust-main/lead';
import { Works } from './cust-main/work';
import { TenderItems } from './cust-main/tender/tenderItems';
...
import { TenderAssocs } from './cust-main/tender/tenderAssoc';
import { PaySchedules } from './cust-main/work-order/paySchedules';
...
{
  return this.http.get(this.API_URL+'/biws_chkWorkDaysExists/'+id+'?Token='+localStorage.getItem('token')).pipe(map(response=>response["0"]));
...
{
  return this.http.get(this.API_URL+'/biws_getWorkOrderDetails/'+id+'?Token='+localStorage.getItem('token'));
}
getCustDetails(id)
{
...
getPaySchedule(id)
{
  return this.http.get<PaySchedules[]>(this.API_URL+'/biws_getPaySchedule/'+id+'?Token='+localStorage.getItem('token')).pipe(map(response=>response["0"]));
}
...
{
  return this.http.get<TenderAssocs[]>(this.API_URL+'/getFinalTenderAssoc/'+id).pipe(map(response=>response["0"]));
}
getFinalTender(id)
{
  return this.http.get<TenderItems[]>(this.API_URL+'/getFinalTender/'+id).pipe(map(response=>response["0"]));
}
woSigned(id)
{
  var data={param1: id} ;
      return this.http.post(this.API_URL+'/woSigned',data,this.options);
}
chkWOSigned(id)
{
  return this.http.get(this.API_URL+'/chkWOSigned/'+id).pipe(map(response=>response["0"]));
}
getWorkDays(id)
{
  return this.http.get(this.API_URL+'/getWorkDays/'+id).pipe(map(response=>response["0"]));
}
getItemRate(id)
{
  return this.http.get(this.API_URL+'/getItemRate/'+id).pipe(map(response=>response["0"]));
}
getStatusNames(tid)
{
  return this.http.get(this.API_URL+'/getStatusNames/'+tid).pipe(map(response=>response["0"]));
}
getAssocStatus(aid, wid)
{
  
  return this.http.get(this.API_URL+'/getAssocStatus/'+aid+'/'+wid).pipe(map(response=>response["0"]));
}
getUserDetails(id)
{
  return this.http.get(this.API_URL+'/getUserDetails/'+id).pipe(map(response=>response["0"]));
}
saveEmailID(values)
{
  var data=JSON.stringify(values);
      return this.http.post(this.API_URL+'/saveEmailID',data,this.options);
}
